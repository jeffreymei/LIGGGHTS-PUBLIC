...
FixStyle(seaice/waves,FixSeaiceWaves)

#else

#ifndef LMP_FIX_SEAICE_WAVES_H
#define LMP_FIX_SEAICE_WAVES_H

#include "fix.h"

namespace LAMMPS_NS {

class FixSeaiceWaves : public Fix {
 public:
  FixSeaiceWaves(class LAMMPS *, int, char **);
  ~FixSeaiceWaves();
  int setmask();
  void init();
  void setup(int);
 ...
 private:
  int wavever;
  // variables used by the jonswap version:
  double *jonspar;
  // variables used by both versions:
  int nw; 
  double **regwave;
  double **wavenum;
  //
